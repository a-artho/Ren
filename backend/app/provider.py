...
SEMANTIC_EXTRACTION_PROMPT_VERSION = "2026-06-30-semantic-exam-cram-first-pass-v3"
...
GLOBAL_EFFORT_CALIBRATION_PROMPT_VERSION = "2026-06-30-global-effort-first-pass-v4"
...
def semantic_prompt() -> str:
    return (
        ...
        "Estimate honest active-study effort for an efficient exam-cram first pass, not full mastery or polished long-term learning. "
        "The student is capable, time-pressured, and trying to reach exam-useful readiness from this source: ready enough to continue exam preparation, answer basic exam questions, and recognize/use the main concepts. "
        "Include time to read/decode the source span, understand key examinable ideas, make only minimal useful annotations if needed, work through essential included examples enough to understand the method, and do a quick self-check against the completion criteria. "
        "Do not include breaks, spaced repetition, later revision sessions, polished note-taking, exhaustive solving of every similar example, optional deep mastery work, extra research, or scheduling safety buffer. "
        "Use effortMinMinutes for the fastest reasonable cram pass if the student is already somewhat familiar or the material is light, while still covering the source span. "
        "Use effortLikelyMinutes for the expected useful cram-pass time for a capable time-pressured student. "
        "Use effortMaxMinutes for cautious first-pass time if the material is unfamiliar, dense, or harder than it first appears, but still not full mastery. "
        "Do not hide uncertainty inside effortLikelyMinutes; use effortMaxMinutes and LOW/MEDIUM confidence to express uncertainty, unfamiliarity, or risk. The app adds scheduling buffer later. "
        "Do not charge time for every slide/page by default; estimate by distinct examinable novelty, cognitive load, and active work. "
        "If the source contains many similar examples or repeated variations, estimate a representative cram pass that teaches the method. Do not assume the student must exhaustively solve every near-duplicate example unless each example introduces a genuinely new concept, method, or exam-relevant variation. "
        "If examples are the main learning content, include enough example work to understand and reproduce the method under exam conditions. "
        "Use 5-minute increments unless a source span is genuinely tiny. Do not compress work to fit the deadline or available time. "
        ...
        "Minutes and cognitive scores are related but not identical. effortLikelyMinutes estimates clock time for the cram pass. difficultyScore, densityScore, and productionDemandScore express mental load. Do not inflate minutes solely because a block is difficult if the source span is short; express heaviness through cognitive scores and, when appropriate, effortMaxMinutes/confidence. "
        ...
        "Create semantically coherent source-ordered study blocks. A block should be a real source/material unit, not an artificial calendar chunk. "
        "Prefer blocks around 10-45 active minutes. Very small admin/reference spans may be 5-10 minutes. Do not inflate a small source span just to reach the preferred range. "
        "A naturally coherent dense block may be longer, but if it contains natural subtopics, methods, examples, or concepts, output them as separate adjacent source-ordered blocks. Do not create artificial time chunks just to fit a schedule. "
...
def global_effort_calibration_prompt() -> str:
    return (
        ...
        "Calibrate effort for the same efficient exam-cram first-pass definition used during extraction, not full mastery. "
        "Do not add scheduling safety buffer to effortLikelyMinutes; the app handles scheduling risk later using effortMaxMinutes and estimateConfidence. "
        ...
        "Text length and page count are only signals, not workload by themselves. Calibrate by distinct examinable novelty, cognitive load, density, production demand, and relationship to earlier blocks. "
        "Keep effortLikelyMinutes as the expected useful cram-pass time for a capable time-pressured student; effortMaxMinutes should carry uncertainty, getting stuck, or deeper review. "
        "Do not preserve inflated per-PDF estimates merely because a block spans many pages if much of it is recap, support material, examples using already-known ideas, or reference/admin content. "
        "Only reduce effort when later material genuinely repeats, recaps, or lightly applies earlier material. Do not reduce effort just because topics are related. "
        "Increase effort only when the per-PDF estimate missed novelty, dense examples, difficult production work, or an important new exam-relevant variation. "
        "If a block is uncertain, prefer adjusting effortMaxMinutes and estimateConfidence rather than making effortLikelyMinutes overly conservative. "
        "Do not compress work to fit the user's deadline or available time. Estimate active-study time for the exam-cram first-pass target, excluding breaks. "
